# Collect images for right hand classes
...
DATA_DIR = './train_classifiers/data/right_hand'
if not os.path.exists(DATA_DIR):
    os.makedirs(DATA_DIR)


# Load classes and define the desired dataset size for each class
with open('map/right_hand_classes.ls', 'rb') as f:
    right_hand_classes = pickle.load(f)
dataset_size = 200
...
if collecting_mode == '1':
    ...
    for right_hand_class in right_hand_classes:
        capture_class_images(right_hand_class)

# Collect data for one class            
elif collecting_mode == '2':
    print(right_hand_classes)
    chosen_class = str(input('Choose the class you want to collect data for: '))
    if chosen_class in right_hand_classes:
